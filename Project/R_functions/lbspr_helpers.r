...
EqSimMod_LB <- function(Stock, Fleet, SizeBins, FitControl=NULL)  {
  ...
  MatLenGTG <- sapply(seq_along(DiffLinfs), function (X) 1.0/(1+exp(LenMids-L50GTG[X])))
  ...
  ## Depletion
  SB0 <- sum(NatLUnFishedPop * MatLenGTG * Weight) 
  SBf <- sum(NatLFishedPop * MatLenGTG * Weight)
  depl <- SBf/SB0
