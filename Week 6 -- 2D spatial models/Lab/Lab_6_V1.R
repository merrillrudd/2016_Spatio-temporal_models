...
setwd("C:\\Git_Projects\\2016_Spatio-temporal_models\\Week 6 -- 2D spatial models\\Lab")
...
Opt3[["run_time"]] = Sys.time() - start_time
...
Opt_spde[["run_time"]] = Sys.time() - start_time
h_spde = Obj$env$spHess(random=TRUE)
report_spde = Obj$report()
sd_spde = sdreport( Obj, bias.correct=TRUE )

# Results
Results = matrix( c(report3$Total_Abundance,sd3$unbiased$value,report_spde$Total_Abundance,sd_spde$unbiased$value), nrow=2)
Results
true_abundance
