...
setwd( "C:/Users/James.Thorson/Desktop/Project_git/2016_Spatio-temporal_models/Week 8 -- Multivariate models/Lab 8/" )

# Load libraries
library(TMB)
library(INLA)
library(RandomFields)
library(TMBdebug) # Only necessary when debugging
...
# Run model
Obj$fn( Obj$par )
Opt = nlminb(start=Obj$par, objective=Obj$fn, gradient=Obj$gr, control=list(eval.max=1e4, iter.max=1e4, trace=1, rel.tol=1e-14) )
...
for(fI in 1:min(n_factors_true,n_factors_estimation)) plot(y=Report$Omega_xf[1:n_stations,fI], x=Data_List$Omega_sf[,fI], xlab="True", ylab="Estimated", main=paste("Factor",fI))
